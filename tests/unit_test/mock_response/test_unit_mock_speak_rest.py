...
input_output = [
    (
        TEXT1,
        SpeakOptions(model="aura-asteria-en", encoding="linear16", sample_rate=24000),
        {
...
def test_unit_mock_speak_rest(text, options, expected_output):
    # Save the options
    ...
    response = deepgram.speak.rest.v("1").stream_memory(input_text, options)
